...
std::array<std::string_view, 7> split_row(const std::string& line, const std::size_t line_number) {
    ...
    for (std::size_t column = 0; column < columns.size(); ++column) {
        ...
        if ((separator == std::string_view::npos) != is_last) {
            throw std::runtime_error("expected seven columns on CSV line " +
...
BookUpdate parse_update(const std::string& line, const std::size_t line_number) {
    ...
    return BookUpdate{
        parse_integer<std::uint32_t>(columns[1], line_number),
        parse_side(columns[4], line_number),
        parse_integer<std::int64_t>(columns[5], line_number),
        parse_integer<std::uint64_t>(columns[6], line_number),
        parse_integer<std::uint64_t>(columns[2], line_number),
        parse_integer<std::uint64_t>(columns[3], line_number),
